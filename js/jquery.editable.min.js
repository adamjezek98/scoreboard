...
(function ($) {
    ...
    $.fn.editable = function (event, callback) {
        ...
        edit.start = function (e) {
            ...
            var old_value = (type === "textarea" ? target.text().replace(/<br( \/)?>/gm, "\n").replace(/&gt;/gm, ">").replace(/&lt;/gm, "<") : target.text()).replace(/^\s+/, "").replace(/\s+$/, "");
            var input = type === "textarea" ? $("<textarea>") : $("<input>");
            input.val(old_value).css("font-size", "100%").css("margin", 0).attr("id", "editable_" + new Date * 1).addClass("editable");
